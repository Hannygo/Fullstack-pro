...
// send sampledata porducts to mongodb
...
export const getProductsByCondition = async (req, res) => {
  try {
    ...
    // if(!products) return next(customError(404, "Can't find products"))
    const Fola = await Product.find({ condition: "Preorder" });
    const allProducts = [products, Fola];
